...
class BasketService{
	...
	async getBasket(){
		try{
			...
			throw new Error("Failed to retireve the basket"+error);
		}
	}	
	...
	async addItemToBasket(item: Product,quantity:1,dispatch:Dispatch){
		try{
			...
			//calculate totals
			const totals = this.calcultateTotals(basket);
			return {basket,totals};
		}catch(error){
			throw new Error("Failed item to basket"+error)
		}
	}
	async remove(itemId:number,dispatch:Dispatch){
		...
		if(basket){
			...
			//check if basket is empty after removing items
			if(basket.items.length ==0){
				//clear tthe basket from the localStorage
	...
	private calcultateTotals(basket: Basket) :  BasketTotals{
		const shipping = 0;
